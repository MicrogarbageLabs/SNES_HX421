...
/* ---- FFT spectrum (for an on-screen analyzer) -------------------------- */
/* Copy up to `n` spectrum band levels (each 0..255, low->high frequency) into
 * `out`; returns the number written. The RISC-V/SNES renders these as bars. The
 * DLL also prints an ASCII spectrum to stderr (visible with -Log) so the meter
 * can be eyeballed host-side before the video path exists. */
