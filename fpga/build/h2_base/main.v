...
`endif
// HX421_TOGGLE=1 -> auto-cycle the mute (both/ch0-only/ch1-only) for an L/R separation test.
`ifndef HX421_TOGGLE
  `define HX421_TOGGLE 1'b0
`endif
...
// ============================================================
//  HX-421 milestone H2 — a signature window served by OUR logic, on the
//  FULL sd2snes_base core (not mini). base is what the FXPak loads for a
//  game: it carries the MCU ROM-load path, so a ROM actually reaches the
//  SNES. mini lacks that path, which is why a mini-derived core configured
//  but then hung on "Loading ...".
//
//  Four reads at $3F:F000-$F003 come from the fabric: 'H','X','4','2'.
...
wire [7:0] HX_SIG_DATA = (SNES_ADDR[1:0] == 2'd0) ? 8'h48   // 'H'
