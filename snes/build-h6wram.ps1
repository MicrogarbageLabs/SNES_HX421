<#
  build-h6wram.ps1 — assemble the execute-from-WRAM test on the 6b.1b core.
...
function Step($m) { Write-Host "build-h6wram: $m" -ForegroundColor Cyan }
if (-not (Get-Command ca65 -ErrorAction SilentlyContinue)) { throw "ca65 not found on PATH." }

$sfc = Join-Path $out "h6_wram.sfc"
ca65 --cpu 65816 -I $here -o "$out\h6_wram.o" "$here\h6_wram.s"
ca65 --cpu 65816 -o "$out\h6_wram_header.o" "$here\h1_header.s"
ld65 -C "$here\h6_1b_lorom.cfg" -o $sfc "$out\h6_wram.o" "$out\h6_wram_header.o"
...
Write-Host ""
Write-Host "Flash the 6b.1b fpga_hx421.bi3, then run h6_wram.sfc." -ForegroundColor Green
