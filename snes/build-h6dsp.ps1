...
function Step($m) { Write-Host "build-h6dsp: $m" -ForegroundColor Cyan }
if (-not (Get-Command ca65 -ErrorAction SilentlyContinue)) { throw "ca65 not found on PATH." }

$sfc = Join-Path $out "h6_dsp.sfc"
ca65 --cpu 65816 -I $here -o "$out\h6_dsp.o" "$here\h6_dsp.s"
ca65 --cpu 65816 -o "$out\h6_dsp_header.o" "$here\h1_header.s"
ld65 -C "$here\h6_1b_lorom.cfg" -o $sfc "$out\h6_dsp.o" "$out\h6_dsp_header.o"
...
Write-Host ""
Write-Host "Flash the 6b.1b fpga_hx421.bi3, then run h6_dsp.sfc." -ForegroundColor Green
