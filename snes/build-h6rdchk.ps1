<#
  build-h6rdchk.ps1 — assemble the execute-from-WRAM test on the 6b.1b core.
...
function Step($m) { Write-Host "build-h6rdchk: $m" -ForegroundColor Cyan }
if (-not (Get-Command ca65 -ErrorAction SilentlyContinue)) { throw "ca65 not found on PATH." }

$sfc = Join-Path $out "h6_rdchk.sfc"
ca65 --cpu 65816 -I $here -o "$out\h6_rdchk.o" "$here\h6_rdchk.s"
ca65 --cpu 65816 -o "$out\h6_rdchk_header.o" "$here\h1_header.s"
ld65 -C "$here\h6_1b_lorom.cfg" -o $sfc "$out\h6_rdchk.o" "$out\h6_rdchk_header.o"
...
Write-Host ""
Write-Host "Flash the 6b.1b fpga_hx421.bi3, then run h6_rdchk.sfc." -ForegroundColor Green
