...
function Step($m) { Write-Host "build-h6mixbw: $m" -ForegroundColor Cyan }
if (-not (Get-Command ca65 -ErrorAction SilentlyContinue)) { throw "ca65 not found on PATH." }

$sfc = Join-Path $out "h6_mixbw.sfc"
ca65 --cpu 65816 -I $here -o "$out\h6_mixbw.o" "$here\h6_mixbw.s"
ca65 --cpu 65816 -o "$out\h6_mixbw_header.o" "$here\h1_header.s"
ld65 -C "$here\h6_1b_lorom.cfg" -o $sfc "$out\h6_mixbw.o" "$out\h6_mixbw_header.o"
