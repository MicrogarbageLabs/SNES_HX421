<#
  build-h6const.ps1 — assemble the execute-from-WRAM test on the 6b.1b core.
...
function Step($m) { Write-Host "build-h6const: $m" -ForegroundColor Cyan }
if (-not (Get-Command ca65 -ErrorAction SilentlyContinue)) { throw "ca65 not found on PATH." }

$sfc = Join-Path $out "h6_const.sfc"
ca65 --cpu 65816 -I $here -o "$out\h6_const.o" "$here\h6_const.s"
ca65 --cpu 65816 -o "$out\h6_const_header.o" "$here\h1_header.s"
ld65 -C "$here\h6_1b_lorom.cfg" -o $sfc "$out\h6_const.o" "$out\h6_const_header.o"
...
if ($w0 -ne 0x1000 -or $w32 -ne 0x1000) {
    throw ("CONST WAVE not at 0x2000: word0={0:X4} word32={1:X4} (expected 1000 / 1000)" -f $w0, $w32)
}
Step ("constant wave 0x1000 at file 0x2000 verified (word0={0:X4} word32={1:X4})" -f $w0, $w32)
...
Write-Host ""
Write-Host "Flash the 6b.1b fpga_hx421.bi3, then run h6_const.sfc." -ForegroundColor Green
